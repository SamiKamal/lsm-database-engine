...
xdescribe("SSTable", () => {
    ...
    test("should initialize with a given file path", () => {
        expect(sstable.filePath).toBe(testFolderPath);
        expect(sstable.currentFile).toBeNull();
    });
...
function getFirstFileDataSync(directory) {
    try {
        const files = fs.readdirSync(directory);
        for (const file of files) {
            const filePath = path.join(directory, file);
            if (fs.statSync(filePath).isFile()) {
                return fs.readFileSync(filePath, 'utf8'); // Read and return file content
            }
        }
